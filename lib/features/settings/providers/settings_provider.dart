  ...
  Future<void> toggle(String key) async {
    ...
    switch (key) {
      case 'push':
        ...
        p.setBool('notif_push', state.pushEnabled);
      case 'followers':
        state = state.copyWith(followers: !state.followers);
        p.setBool('notif_followers', state.followers);
      case 'likes':
        state = state.copyWith(likes: !state.likes);
        p.setBool('notif_likes', state.likes);
      case 'dislikes':
        state = state.copyWith(dislikes: !state.dislikes);
        p.setBool('notif_dislikes', state.dislikes);
      case 'comments':
        state = state.copyWith(comments: !state.comments);
        p.setBool('notif_comments', state.comments);
      case 'replies':
        state = state.copyWith(replies: !state.replies);
        p.setBool('notif_replies', state.replies);
    }
  }
}
